...
export class SeguidorRepositoryNeo4j implements SeguidorRepository {
  ...
  private toUsuarioSeguimientoDomain(
    usuarioSeguimiento: RecordShape,
  ): UsuarioSeguimiento {
    ...
    try {
      ...
      if (userNode === null) {
        throw new EntityNotFoundError(
          'Usuario de seguimiento no encontrado con id ' + id,
        );
      }
  ...
  async getAll(): Promise<UsuarioSeguimiento[]> {
    ...
    try {
      ...
      return userNodes.map((userNode) =>
        this.toUsuarioSeguimientoDomain(userNode),
      );
    } catch (error) {
      await txc.rollback();
  ...
  async update(
    ...
    try {
      ...
      throw new RepositoryError(
        'Error al actualizar el usuario de seguimiento',
      );
    } finally {
      await session.close();
    }
  ...
  async getUsuariosSeguidos(id: string): Promise<UsuarioSeguimiento[]> {
    ...
    try {
      ...
      return seguidores.map((seguidor) =>
        this.toUsuarioSeguimientoDomain(seguidor),
      );
    } catch (error) {
      await txc.rollback();
